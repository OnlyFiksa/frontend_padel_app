...
class _ExploreScreenState extends State<ExploreScreen> {
  ...
  // Kategori default sebelum ditimpa database
  List<String> categories = ['Semua']; 
  
  List<dynamic> allVenues = [];      // Menyimpan semua data asli dari API
  List<dynamic> filteredVenues = []; // Menyimpan data yang sudah di-filter (yang tampil di layar)
  ...
  Future<void> _fetchVenues() async {
    try {
      ...
      if (response.statusCode == 200 && response.data['status'] == 200) {
        setState(() {
          // Tangkap categories dan venues dari struktur JSON baru
          categories = List<String>.from(response.data['data']['categories']);
          allVenues = response.data['data']['venues'] ?? [];
          filteredVenues = allVenues; // Awal mula tampilkan semua
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  // LOGIKA FILTER KETIKA LOKASI DI-KLIK
  void _onCategorySelected(int index) {
    setState(() {
      selectedCategoryIndex = index;
      
      if (index == 0) {
        // Kalau pilih "Semua", kembalikan list ke asalnya
        filteredVenues = allVenues;
      } else {
        // Kalau pilih kota spesifik (Misal: Jakarta Selatan), saring list-nya!
      ...
      body: SafeArea(
        child: Column(
          ...
          children: [
            ...
            _buildCategoryChips(), // Baris Lokasi
            _buildSectionTitle(),
            
            Expanded(
              child: _isLoading 
                  ? Center(child: CircularProgressIndicator(color: primaryColor))
                  // Tampilkan yang sudah di filter
                  : _buildVenueList(), 
            ),
          ],
        ),
  ...
  Widget _buildCategoryChips() {
    // Kalau category masih cuma 'Semua', berarti API belum selesai loading
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return GestureDetector(
            onTap: () => _onCategorySelected(index), // Manggil fungsi filter
            child: Container(
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              ...
                              const SizedBox(width: 4),
                              Text('${venue['location']} • ${venue['distance']}', style: const TextStyle(fontSize: 12, color: Colors.grey, fontWeight: FontWeight.w500)),
                            ],
                          ),
                        ],
                      ),
